...
public class LsbDecoder implements Decoder {
    ...
    @Override
    public byte[] decode(BufferedImage image, Charset charset) {
        return decode(image, charset, ';');
    }

    @Override
    public byte[] decode(BufferedImage image, char delimiter) {
        return decode(image, StandardCharsets.UTF_8, delimiter);
    }

    @Override
    public byte[] decode(BufferedImage image, Charset charset, char delimiter) {
